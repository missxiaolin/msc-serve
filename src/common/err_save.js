        ...
        case error.PERFORMANCE:
          for (let key in item.metrics) {
            performanceModel.save({
              ...
              textValue: (Util.getInstance().isType().isPlainObject(item.metrics[key].value) || Util.getInstance().isArray(item.metrics[key].value)) && item.metrics[key].value ? JSON.stringify(item.metrics[key].value) : '',
